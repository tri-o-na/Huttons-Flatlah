...
router.get('/', async (req, res) => {
    try {
        const towns = await server_1.prisma.property.groupBy({
            by: ['town'],
            _count: { _all: true },
            _avg: { resale_price: true, psf: true }
        });
        const townData = await Promise.all(towns.map(async (t) => {
            const flatTypes = await server_1.prisma.property.groupBy({
                by: ['flat_type'],
                where: { town: t.town },
                _count: { _all: true }
            });
...
router.get('/:town', async (req, res) => {
    try {
        ...
        const properties = await server_1.prisma.property.findMany({ where: { town } });
        ...
        const topStreets = (await server_1.prisma.property.groupBy({
            by: ['street_name'],
            where: { town },
            _count: { _all: true },
        ...
        const priceByTypeAndYear = properties.reduce((acc, p) => {
            ...
            if (!acc[p.flat_type])
                acc[p.flat_type] = {};
            if (!acc[p.flat_type][year])
                acc[p.flat_type][year] = [];
