...
function formatSuggestionPath(town, street, block) {
    ...
    try {
        ...
        const where = {
            OR: [
                ...
                { flat_type: { contains: q, mode: 'insensitive' } }
            ]
        };
        const suggestionGroups = await server_1.prisma.property.groupBy({
...
router.get('/search', async (req, res) => {
    try {
        ...
        const town = normalizeParam(req.query.town);
        const street = normalizeParam(req.query.street);
        const block = req.query.block;
        const storeyRange = req.query.storeyRange;
        const minPrice = parseRange(req.query.minPrice);
        const maxPrice = parseRange(req.query.maxPrice);
        const minArea = parseFloat(req.query.minArea || '0');
        const maxArea = parseFloat(req.query.maxArea || '0');
        ...
        if (block)
            where.block = block;
        if (storeyRange)
            where.storey_range = storeyRange;
        if (minPrice || maxPrice) {
            where.resale_price = {};
            if (minPrice)
                where.resale_price.gte = minPrice;
            if (maxPrice)
                where.resale_price.lte = maxPrice;
        }
        if (minArea || maxArea) {
            where.floor_area_sqm = {};
            if (!Number.isNaN(minArea) && minArea > 0)
                where.floor_area_sqm.gte = minArea;
            if (!Number.isNaN(maxArea) && maxArea > 0)
                where.floor_area_sqm.lte = maxArea;
        }
        let orderBy;
        ...
        const properties = await server_1.prisma.property.groupBy({
        ...
        const totalGroups = (await server_1.prisma.property.groupBy({
            by: ['town', 'street_name', 'block'],
            where,
            _count: { _all: true }
        })).length;
...
router.get('/:town/:street/:block', async (req, res) => {
    try {
        ...
        const avgPriceByFlatType = Object.entries(properties.reduce((acc, p) => {
            if (!acc[p.flat_type])
                acc[p.flat_type] = { total: 0, count: 0 };
        ...
        const storeyRangeAvg = Object.entries(properties.reduce((acc, p) => {
            if (!acc[p.storey_range])
                acc[p.storey_range] = { total: 0, count: 0 };
            acc[p.storey_range].total += p.resale_price;
            acc[p.storey_range].count += 1;
            return acc;
        }, {})).map(([storey_range, data]) => ({
            storey_range,
            avgPrice: data.total / data.count
        }));
        ...
        const nearbyStreets = (await server_1.prisma.property.groupBy({
            by: ['street_name'],
            where: { town },
            _count: { _all: true },
